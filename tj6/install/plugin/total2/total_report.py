...
import public
from total_main import total_main
...
def get_overview_data(site, query_date):
    ...
    cycle_data = tm.get_site_overview_sum_data(site, start_date, end_date)
    report_data = {}
    if not cycle_data:
        return report_data
...
def get_ip_data(site, query_date):
    ...
    get.top = 20
    select_data = tm.get_ip_stat(get, flush=False)
...
def get_uri_data(site, query_date):
    ...
    get.query_date = query_date
    get.top = 20
    select_data = tm.get_uri_stat(get, flush=False)
    data = select_data["data"]
    uri_data = []
    for data_line in data:
        count = data_line["count"]
...
def get_area_data(site, query_date):
    ...
    get.query_date = query_date
    get.top = 20
    tm.generate_area_stat(site, query_date)
    select_data = tm.get_area_stat(get, flush=False, format_count=True)
    area_data = select_data["data"]
    return area_data
...
def get_referer_data(site, query_date):
    ...
    get.query_date = query_date
    get.top = 20
    select_data = tm.get_referer_stat(get, flush=False, format_count=True)
...
def generate_week_report(site, test=False):
    try:
        report_conn = None
        report_cur = None
        init_db()
        ...
        report_conn.commit()
        if test:
            os.system("cp -f {} {}".format(file_name, "/www/wwwroot/192.168.1.60/"))
...
def generate_month_report(site, base_time=None, test=False):
    try:
        report_conn = None
        report_cur = None
...
def automatic_generate_report():
    ...
    if len(generator) == 0:
        print(tool.getMsg("REPORT_GENERATED"))
        return True
...
if __name__ == "__main__":
    # TODO 控制日志分析只进行一次
    # parse_logs_data("www.bt.cn", "20210701-20210702")
    # print(get_spider_data("www.bt.cn", "20210720-20210730"))
    # print(get_ip_data("www.bt.cn", "20210720-20210730"))
    # print(get_area_data("www.bt.cn", "20210725-20210730"))
    # print(get_referer_data("www.bt.cn", "20210715-20210722"))
    # get_last_week()

    # clear_report()
    # generate_month_report("www.bt.cn", test=True)
    # generate_week_report("192.168.1.60", test=True)
