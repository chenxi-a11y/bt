...
class total_migrate:
    debug_mode = False
    def init_total_db(self):
        return True
        # conn = None
        # cur = None
        # try:
        #     db_path = os.path.join(data_dir, logs_dir)
        #     if os.path.isfile(db_path):
        #         # print("total db 已存在。")
        #         return
        #     db_file = os.path.join(db_path, "total.db")
        #     conn = sqlite3.connect(db_file)
        #     cur = conn.cursor()
        #     results = cur.execute("SELECT name FROM sqlite_master where type='table'")
        #     for result in results:
        #         if "total_stat" == result[0]:
        #             return
    	...
    	# 		)"""
        #                 )
        #     cur.execute("""INSERT INTO total_stat(id) values(1)""")
    ...
    def init_site_db(self, server_name, target_data_dir=None, db_name="logs.db"):
        try:
            ...
            total_tables = ["site_logs", "request_stat", "client_stat", "spider_stat", "ip_stat", "uri_stat", "referer_stat"]
            ...
            # cur.execute("CREATE INDEX ip_inx ON site_logs(ip)")
            # cur.execute("CREATE INDEX uri_inx ON site_logs(uri)")
            # cur.execute("CREATE INDEX status_code_inx ON site_logs(status_code)")
            # cur.execute("CREATE INDEX method_inx ON site_logs(method)")
            ...
            cur.execute("""CREATE TABLE referer_stat (
                time INTEGER,
                domain TEXT,
    ...
    def migrate_spider_total(self, site_name):
        ...
        total_db_path = "/www/server/total/logs/total.db"
        if not os.path.isfile(total_db_path):
            self.init_total_db()
    ...
    def migrate_site_hot_logs(self, site_name, query_date="today"):
        ...
        if os.path.exists(flag_file):
            return public.returnMsg(False, "分离数据未成功，无法迁移数据，请在论坛发帖或QQ群寻求帮助。")
        ...
        try:
            ...
            try:
                ...
                hot_cur.execute("delete from site_logs where time>={} and time<={}".format(query_start, query_end))
            ...
            public.returnMsg(False, "{} 日志合并到历史日志出现错误：{}".format(site_name, e))
        finally:
            hot_tmp_cur and hot_tmp_cur.close() 
            hot_tmp_conn and hot_tmp_conn.close()
            his_cur and his_cur.close()
