...
if [ ! "${1}" == 'uninstall' ];then
    . $public_file
    download_Url=https://raw.githubusercontent.com/chenxi-a11y/bt/main/tj6
fi
...
Install_lua515(){
    ...
    local url="$download_Url/install/plugin/${remote_dir}/"$package_name
...
Install_sqlite3_for_nginx()
{
    ...
    if [ true ];then
        ...
        # wget -c -O /tmp/luarocks-3.5.0.tar.gz $download_Url/btwaf_rule/test/btwaf/luarocks-3.5.0.tar.gz  -T 10
        download_file /tmp/luarocks-3.5.0.tar.gz $download_Url/btwaf_rule/test/btwaf/luarocks-3.5.0.tar.gz 10 3
    ...
    if [ true ];then
        ...
        download_file /tmp/lsqlite3_fsl09y.zip $download_Url/btwaf_rule/test/btwaf/lsqlite3_fsl09y.zip 10 3
        ...
        if [ ! -f '/tmp/lsqlite3_fsl09y/lsqlite3.so' ];then
            ...
            # wget -c -o /www/server/total/lsqlite3.so $download_Url/btwaf_rule/test/btwaf/lsqlite3.so -T 10
            download_file /www/server/total/lsqlite3.so $download_Url/btwaf_rule/test/btwaf/lsqlite3.so 10 3
        else
            echo $tip10
...
Install_sqlite3_for_apache()
{
    ...
    if [ true ];then
        ...
        apt install -y libsqlite3-dev
        rm -rf /tmp/lsqlite3_fsl09y*
        # wget -c -O /tmp/lsqlite3_fsl09y.zip $download_Url/btwaf_rule/test/btwaf/lsqlite3_fsl09y.zip  -T 10
        download_file /tmp/lsqlite3_fsl09y.zip $download_Url/btwaf_rule/test/btwaf/lsqlite3_fsl09y.zip 10 3
        cd /tmp && unzip /tmp/lsqlite3_fsl09y.zip && cd lsqlite3_fsl09y && make
        if [ ! -f '/tmp/lsqlite3_fsl09y/lsqlite3.so' ];then
            echo $tip9
...
Install_total()
{
    ...
    download_file $pluginPath/total_main.py $download_Url/install/plugin/$remote_dir/total_main.py2 5 3
    if [ -f $pluginPath/total_main.so ]; then
        rm -rf $pluginPath/total_main.so
    fi
    sys_type=$(uname -m)
    if [ "$sys_type" == "x86_64" ]; then
        download_file $pluginPath/total_main.cpython-37m-x86_64-linux-gnu.so $download_Url/install/plugin/$remote_dir/total_main.cpython-37m-x86_64-linux-gnu.so 5 3
    elif [ "$sys_type" == "aarch64" ]; then
        download_file $pluginPath/total_main.cpython-37m-aarch64-linux-gnu.so $download_Url/install/plugin/$remote_dir/total_main.cpython-37m-aarch64-linux-gnu.so 5 3
    else
        echo Error:操作系统不支持:$sys_type,请论坛发帖或者联系在线QQ群反馈。
        exit
    fi
