...
Node_Check(){
	...
	[ ! -f $public_file ] && wget -O $public_file 域名/install/public.sh -T 5;
	. $public_file
...
Install_Psr(){
	...
	if [ ! -f "${PsrFile}" ];then
		psrVersion="1.0.0"
		wget -O psr-${psrVersion}.tgz ${download_Url}/src/psr-${psrVersion}.tgz
...
Install_Phalcon(){
	...
	if [ ! -f "${extFile}" ];then
		...
		if [ "${version}" -ge "72" ]; then
			Install_Psr
			phalconVer="4.0.5"
			wget -O phalcon-${phalconVer}.tgz ${download_Url}/src/phalcon-${phalconVer}.tgz
			tar zxvf phalcon-${phalconVer}.tgz
			rm -f phalcon-${phalconVer}.tgz
			cd phalcon-${phalconVer}
		else
			phalconVer="3.4.4"
			wget ${download_Url}/src/cphalcon-${phalconVer}.tar.gz
