...
Install_JavaJdk() {
    if [ "${PM}" == "apt-get" ]; then
        if [ ! -f "/www/server/jvm/java-1.8.0/bin/java" ]; then
            mkdir -p /www/server/jvm
            cd /www/server/jvm
        ...
        /www/server/jvm/java-1.8.0/bin/java -version
        if [ "$?" -ne "0" ]; then
            echo "jdk install failed"
            rm -rf /www/server/jvm/java-1.8.0
            exit
        fi
        export JAVA_HOME=/www/server/jvm/java-1.8.0
        jdk_path='/www/server/jvm/java-1.8.0'
        return
    fi
    if [ ! -d "$jdk_path" ]; then
...
Install_Jsvs(){
	...
	if [ "${version}" == "7" ];then
		...
		cd commons-daemon-1.3.0-native-src/unix
	fi
