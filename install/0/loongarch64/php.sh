...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh 域名/install/public.sh -T 20;
...
# mkdir -p /usr/local/pcre2/bin
# mkdir -p /usr/local/pcre2/include
# mkdir -p /usr/local/pcre2/lib
# mkdir -p /usr/local/pcre2/lib/pkgconfig
# ln -sf /usr/bin/pcre2-config /usr/local/pcre2/bin/pcre-config
# ln -sf /usr/include/pcre2.h /usr/local/pcre2/include/pcre.h
# ln -sf /usr/lib64/pkgconfig/libpcre2-16.pc /usr/local/pcre2/lib/pkgconfig/libpcre-16.pc
# ln -sf /usr/lib64/pkgconfig/libpcre2-32.pc /usr/local/pcre2/lib/pkgconfig/libpcre-32.pc
# ln -sf /usr/lib64/pkgconfig/libpcre2-8.pc /usr/local/pcre2/lib/pkgconfig/libpcre-8.pc
# ln -sf /usr/lib64/pkgconfig/libpcre2-posix.pc /usr/local/pcre2/lib/pkgconfig/libpcre-posix.pc
...
Install_Curl_New(){
	if [ ! -f "/usr/local/curl_2/bin/curl" ];then
		...
		./configure --prefix=/usr/local/curl_2 --enable-ldap --enable-ldaps --with-brotli --with-libssh2 --with-libssh --enable-ares --with-gssapi --without-nss --enable-smb --with-libidn2 --with-ssl=/usr/local/openssl111 --build=arm-linux
		[ $? -ne 0 ] && Error_Msg
		make -j${cpuCore}
		make install
		cd ..
...
Install_Curl2(){
	...
	if [[ "${LibCurlVer}" -le "60" ]]; then
		if [ ! -f "/usr/local/curl2/bin/curl" ];then
			...
			cd curl-${curlVer}
			./configure --prefix=/usr/local/curl2 --enable-ares --without-nss --with-ssl=/usr/local/openssl --build=arm-linux
			make -j${cpuCore}
			make install
			cd ..
			rm -rf curl*
...
Install_Icu4c(){
	...
	if [ ! -f "/usr/bin/icu-config" ] || [ "${icu4cVer:0:2}" -gt "60" ];then
		...
		cd icu/source
		./configure --prefix=/usr/local/icu --build=arm-linux
		make -j${cpuCore}
		make install
...
Install_Onig(){
	...
	if [ -z "${onigCheck}" ];then
		...
		cd onig-${onigVer}
		./configure --prefix=/usr/local/onig --build=arm-linux
		make -j${cpuCore} 
		make install
		cd ..
		rm -rf onig-${onigVer}*
...
Install_Libsodium(){
	...
	if [ ! -f "/usr/local/libsodium/lib/libsodium.so" ];then
		...
		cd libsodium-stable
		./configure --prefix=/usr/local/libsodium --build=arm-linux
		make -j${cpuCore}
		make install
		cd ..
		rm -f libsodium-${libsodiumVer}-stable.tar.gz
...
Install_Configure(){
 ...
 ${ENABLE_WEBP} ${ENABLE_OPCACHE} ${CONFIGURE_BUILD_TYPE} ${i_make_args}
		...
		if [ -z "${onigCheck}" ] || [ -z "${sodiumCheck}" ] || [ "${ALI_OS}" ];then
			#export PKG_CONFIG_PATH="/usr/local/pcre2/lib/pkgconfig:/usr/local/onig/lib/pkgconfig:/usr/local/libsodium/lib/pkgconfig:$PKG_CONFIG_PATH"
			export PKG_CONFIG_PATH="/usr/local/onig/lib/pkgconfig:/usr/local/libsodium/lib/pkgconfig:$PKG_CONFIG_PATH"
			#export PKG_CONFIG_LIBDIR="/usr/local/onig/lib/pkgconfig:/usr/local/libsodium/lib/pkgconfig:$PKG_CONFIG_LIBDIR"
			#export PKG_CONFIG_PATH=/usr/local/pcre2_5/lib/pkgconfig:$PKG_CONFIG_PATH
		fi
...
# 		./configure --prefix=${php_setup_path} --with-config-file-path=${php_setup_path}/etc --enable-fpm --with-fpm-user=www --with-fpm-group=www --enable-mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-iconv-dir --with-freetype --with-jpeg --with-zlib --with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl --enable-mbregex --enable-mbstring --enable-intl --enable-pcntl --enable-ftp --enable-gd --with-openssl --with-mhash --enable-pcntl --enable-sockets --with-xmlrpc  --enable-soap --with-gettext --disable-fileinfo --enable-opcache --with-sodium=/usr/local/libsodium --with-webp --build=arm-linux --with-pcre-regex=/usr/local/pcre2_5 ${i_make_args}
	fi
...
Install_Zip_ext(){
	...
	./configure --with-php-config=${php_setup_path}/bin/php-config --build=arm-linux
	make && make install
	cd ../../
