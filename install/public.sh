...
get_node_url(){
	nodes=(https://raw.githubusercontent.com/chenxi-a11y/bt/main);
	...
	if [ -z "$NODE_URL" ];then
		...
		if [ -z "$NODE_URL" ];then
			NODE_URL='https://raw.githubusercontent.com/chenxi-a11y/bt/main';
		fi
	fi
	rm -f $tmp_file1
	rm -f $tmp_file2
...
send_check(){
	...
	curl -sS --connect-timeout 3 -m 60 https://raw.githubusercontent.com/chenxi-a11y/bt/main/api/panel/notpro?version=$p_version
	NODE_URL=""
	exit 0;
}
GetSysInfo(){
