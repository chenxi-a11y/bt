...
System_Lib(){
	if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ] ; then
		...
		Pack="gcc gcc-c++ libsodium-devel sqlite-devel oniguruma-devel libwebp-devel libvpx-devel"
...
Install_Curl_New(){
	if [ ! -f "/usr/local/curl2/bin/curl" ];then
		...
		tar -zxf curl-${curlVersion}.tar.gz
		cd curl-${curlVersion}
		rm -rf /usr/local/curl2
		./configure --prefix=/usr/local/curl2 --enable-ldap --enable-ldaps --with-brotli --with-libssh2 --with-libssh --enable-ares --with-gssapi --without-nss --enable-smb --with-libidn2 --with-ssl=/usr/local/openssl111
		make -j${cpuCore}
		make install
		cd ..
		rm -f curl-${curlVersion}.tar.gz
...
Install_Libzip(){
	if [ "${PM}" == "yum" ];then
		el=$(cat /etc/redhat-release|grep CentOS|grep -Eo '([0-9]+\.)+[0-9]+'|grep -Eo '^[0-9]')
		if [ "${el}" == "8" ];then
...
Install_Zip_ext(){
	...
	./configure --with-php-config=${php_setup_path}/bin/php-config --enable-phalcon
	make && make install
	cd ../../
...
SetPHPMyAdmin()
{
	...
	PHPVersion=""
	for phpV in 52 53 54 55 56 70 71 72 73 74
	do
...
Remove_Src(){
	cd ${php_setup_path}
	ls |grep -v ext|xrags rm -rf
	cd ext
	rm -rf reflection standard
}
Uninstall_PHP()
{
