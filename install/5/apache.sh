...
if [ "${Centos8Check}" ] || [ "${CentosStream8Check}" ];then
	wget -O apache.sh $download_Url/install/0/8/apache.sh -T 20
	bash apache.sh $1 $2
	exit;
fi
...
System_Lib(){
	if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ] ; then
		...
		Pack="gcc g++ lua5.1 lua5.1-dev lua-cjson lua-socket"
		${PM} install ${Pack} -y
		ENABLE_LUA="--enable-lua"
	fi
}
...
if [ "$actionType" == "install" ] || [ "${actionType}" == "update" ];then
	armCheck=$(cat /etc/issue|grep Raspbian)
	[ -z "${armCheck}" ] && Install_OpenSSL
	Install_Nghttp2
	if [ "$version" == "2.2" ];then
		Install_Apache_22
	else
