...
Install_smbclient()
{
  ...
  if [ "${PM}" == "yum" ] || [ "${PM}" == "dnf" ];then
    ...
    if [ "$os_version" == "8" ] && [ "$is_oracle" == "oracle" ];then
      ...
      if [ "$is_aarch64" != "" ];then
        os_ver="aarch64"
        wget -O libsmbclient.rpm $download_Url/src/libsmbclient-$lib_ver.$el_ver.$os_ver.rpm
        wget -O libsmbclient-devel.rpm $download_Url/src/libsmbclient-devel-$lib_ver.$el_ver.$os_ver.rpm
  ...
  if [ ! -f "${extFile}" ];then
  	wget $download_Url/src/smbclient-$smbclient_Version.tgz
